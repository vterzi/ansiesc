...
class _Consts:
    ...
    def __repr__(self):
        return '(' + ', '.join(key for key in dir(self)
                               if not key.startswith('_')) + ')'


C0 = _Consts(lambda key, value: _StrConst(value[0], value[1]), {
        'NUL': ('\0', 'Null'),
...
C1 = _Consts(lambda key, value: _StrConst(C0.ESC + value[0], value[1]), {
        ...
        'HTJ': ('I', 'Character Tabulation with Justification (Horizontal'
...
ANSIControl = _Consts(lambda key, value: _StrConst(C1.CSI + value[0],
                                                   value[1]), {
        'CUU': ('{}A', 'Cursor Up'),
        'CUD': ('{}B', 'Cursor Down'),
...
ANSIText = _Consts(lambda key, value: _IntConst(value, key), {
        'RESET': 0,
        'BOLD': 1,
        'HIGH_INTENSITY': 1,
        'FAINT': 2,
...
ANSIColor = _Consts(lambda key, value: _IntConst(value, key), {
        'BLACK': 0,
        'RED': 1,
        'GREEN': 2,
        'YELLOW': 3,
